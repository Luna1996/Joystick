...
static const struct xpad_device {
  ...
  u8 xtype;
    ...
    {0x045e, 0x028e, "XBOX 360 PAD - CSE 522S", 0, XTYPE_XBOX360},
...
#define XBOXONE_INIT_PKT(_vid, _pid, _data)                   \
  {                                                           \
    .idVendor = (_vid), .idProduct = (_pid), .data = (_data), \
 ...
 */
static const u8 xboxone_fw2015_init[] = {0x05, 0x20, 0x00, 0x01, 0x00};
 ...
 */
static const u8 xboxone_hori_init[] = {0x01, 0x20, 0x00, 0x09, 0x00, 0x04, 0x20,
 ...
 */
static const u8 xboxone_pdp_init1[] = {0x0a, 0x20, 0x00, 0x03,
                                       0x00, 0x01, 0x14};
 ...
 * (0x0e6f:0x02a4), (0x0e6f:0x02a6).
 */
static const u8 xboxone_pdp_init2[] = {0x06, 0x20, 0x00, 0x02, 0x01, 0x00};

/*
 * A specific rumble packet is required for some PowerA pads to start
 * sending input reports. One of those pads is (0x24c6:0x543a).
 */
static const u8 xboxone_rumblebegin_init[] = {0x09, 0x00, 0x00, 0x09, 0x00,
                                              0x0F, 0x00, 0x00, 0x1D, 0x1D,
                                              0xFF, 0x00, 0x00};
 ...
 */
static const u8 xboxone_rumbleend_init[] = {0x09, 0x00, 0x00, 0x09, 0x00,
...
static void xpad360w_process_packet(struct usb_xpad *xpad, u16 cmd,
  ...
  /* Valid pad data */
  if (data[1] != 0x1) return;

  rcu_read_lock();
  dev = rcu_dereference(xpad->x360w_dev);
  if (dev) xpad360_process_packet(xpad, dev, cmd, &data[4]);
  rcu_read_unlock();
}
...
static void xpadone_process_packet(struct usb_xpad *xpad, u16 cmd,
  ...
  if (data[0] == 0X07) {
     ...
     */
    if (data[1] == 0x30) xpadone_ack_mode_report(xpad, data[2]);
...
static bool xpad_prepare_next_init_packet(struct usb_xpad *xpad) {
  ...
  if (xpad->xtype != XTYPE_XBOXONE) return false;
...
static bool xpad_prepare_next_out_packet(struct usb_xpad *xpad) {
  ...
  if (xpad_prepare_next_init_packet(xpad)) return true;
...
static int xpad_led_probe(struct usb_xpad *xpad) {
  ...
  if (xpad->xtype != XTYPE_XBOX360 && xpad->xtype != XTYPE_XBOX360W) return 0;
...
static int xpad_init_input(struct usb_xpad *xpad) {
  ...
  if (error) goto err_free_input;

  error = xpad_led_probe(xpad);
  if (error) goto err_destroy_ff;

  error = input_register_device(xpad->dev);
  if (error) goto err_disconnect_led;
...
static int xpad_probe(struct usb_interface *intf,
  ...
  if (intf->cur_altsetting->desc.bNumEndpoints != 2) return -ENODEV;
  ...
  if (xpad->xtype == XTYPE_UNKNOWN) {
    ...
    if (dpad_to_buttons) xpad->mapping |= MAP_DPAD_TO_BUTTONS;
    if (triggers_to_buttons) xpad->mapping |= MAP_TRIGGERS_TO_BUTTONS;
    if (sticks_to_null) xpad->mapping |= MAP_STICKS_TO_NULL;
  }
  ...
  if (xpad->xtype == XTYPE_XBOX360W) {
    ...
    if (error) goto err_deinit_output;
    /*
...
static void xpad_disconnect(struct usb_interface *intf) {
  ...
  if (xpad->xtype == XTYPE_XBOX360W) xpad360w_stop_input(xpad);
