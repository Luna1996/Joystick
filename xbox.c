...
static void joydev_pass_event(struct joydev_client *client,
  ...
  if (client->startup == joydev->nabs + joydev->nkey) {
    ...
    if (client->tail == client->head) client->startup = 0;
  }
...
static void joydev_close_device(struct joydev *joydev) {
  ...
  if (joydev->exist && !--joydev->open) input_close_device(&joydev->handle);
...
static int joydev_open(struct inode *inode, struct file *file) {
  struct joydev *joydev = container_of(inode->i_cdev, struct joydev, cdev);
  struct joydev_client *client;
  int error;
...
static ssize_t joydev_0x_read(struct joydev_client *client,
                              struct input_dev *input, char __user *buf) {
  struct joydev *joydev = client->joydev;
  ...
  if (copy_to_user(buf, &data, sizeof(struct JS_DATA_TYPE))) return -EFAULT;
...
static ssize_t joydev_read(struct file *file, char __user *buf, size_t count,
                           loff_t *ppos) {
  struct joydev_client *client = file->private_data;
  struct joydev *joydev = client->joydev;
  ...
  if (count < sizeof(struct js_event)) return -EINVAL;
  ...
  retval = wait_event_interruptible(
      joydev->wait, !joydev->exist || joydev_data_pending(client));
  if (retval) return retval;
...
static int joydev_handle_JSIOCSAXMAP(struct joydev *joydev, void __user *argp,
                                     size_t len) {
  __u8 *abspam;
  int i;
  int retval = 0;
  ...
  if (IS_ERR(abspam)) return PTR_ERR(abspam);
  ...
  for (i = 0; i < joydev->nabs; i++) joydev->absmap[joydev->abspam[i]] = i;
...
static int joydev_handle_JSIOCSBTNMAP(struct joydev *joydev, void __user *argp,
                                      size_t len) {
  __u16 *keypam;
  int i;
  int retval = 0;
  ...
  if (IS_ERR(keypam)) return PTR_ERR(keypam);
  ...
  for (i = 0; i < joydev->nkey; i++) joydev->keymap[keypam[i] - BTN_MISC] = i;
...
static int joydev_ioctl_common(struct joydev *joydev, unsigned int cmd,
                               void __user *argp) {
  struct input_dev *dev = joydev->handle.dev;
  size_t len;
  int i;
...
#ifdef CONFIG_COMPAT
static long joydev_compat_ioctl(struct file *file, unsigned int cmd,
                                unsigned long arg) {
  struct joydev_client *client = file->private_data;
  struct joydev *joydev = client->joydev;
  ...
  switch (cmd) {
    case JS_SET_TIMELIMIT:
      ...
      if (retval == 0) joydev->glue.JS_TIMELIMIT = tmp32;
      break;
...
static long joydev_ioctl(struct file *file, unsigned int cmd,
                         unsigned long arg) {
  struct joydev_client *client = file->private_data;
  struct joydev *joydev = client->joydev;
  void __user *argp = (void __user *)arg;
  ...
  switch (cmd) {
    ...
    case JS_SET_ALL:
      retval = copy_from_user(&joydev->glue, argp, sizeof(joydev->glue))
                   ? -EFAULT
                   : 0;
      break;
    ...
    case JS_GET_ALL:
      retval =
          copy_to_user(argp, &joydev->glue, sizeof(joydev->glue)) ? -EFAULT : 0;
      break;
...
static void joydev_cleanup(struct joydev *joydev) {
  ...
  if (joydev->open) input_close_device(handle);
}
...
static bool joydev_dev_is_blacklisted(struct input_dev *dev) {
  ...
  for (id = joydev_blacklist; id->flags; id++) {
    if (input_match_device_id(dev, id)) {
      dev_dbg(&dev->dev, "joydev: blacklisting '%s'\n", dev->name);
      return true;
    }
  }
...
static bool joydev_dev_is_absolute_mouse(struct input_dev *dev) {
  ...
  if (!bitmap_equal(jd_scratch, dev->evbit, EV_CNT)) return false;

  bitmap_zero(jd_scratch, ABS_CNT);
  __set_bit(ABS_X, jd_scratch);
  __set_bit(ABS_Y, jd_scratch);
  if (!bitmap_equal(dev->absbit, jd_scratch, ABS_CNT)) return false;
  ...
  if (!bitmap_equal(dev->keybit, jd_scratch, KEY_CNT)) return false;

  /*
   * Amiga joystick (amijoy) historically uses left/middle/right
   * button events.
   */
  if (dev->id.bustype == BUS_AMIGA) return false;
...
static bool joydev_match(struct input_handler *handler, struct input_dev *dev) {
  ...
  if (joydev_dev_is_blacklisted(dev)) return false;

  /* Avoid absolute mice */
  if (joydev_dev_is_absolute_mouse(dev)) return false;
...
static int joydev_connect(struct input_handler *handler, struct input_dev *dev,
  ...
  if (dev_no < JOYDEV_MINOR_BASE + JOYDEV_MINORS) dev_no -= JOYDEV_MINOR_BASE;
  ...
  if (error) goto err_free_joydev;

  cdev_init(&joydev->cdev, &joydev_fops);

  error = cdev_device_add(&joydev->cdev, &joydev->dev);
  if (error) goto err_cleanup_joydev;
